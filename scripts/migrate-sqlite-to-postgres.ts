...
async function migrate() {
  ...
  info(
    `SQLite file size: ${(statSync(resolvedPath).size / 1024 / 1024).toFixed(2)} MB`
  );
  console.log("");
  ...
  try {
        ...
        try {
          await prisma.rule.upsert({
            ...
            create: {
              ...
              relatedRules: rule.related_rules
                ? JSON.parse(rule.related_rules)
                : null,
    ...
    if (tables.some((t) => t.name === "sanity_triggers")) {
      ...
      const triggers = sqlite
        .prepare("SELECT * FROM sanity_triggers")
        .all() as any[];
      for (const trigger of triggers) {
        try {
          ...
          if (!e.code || e.code !== "P2002") {
            warn(
              `  Failed to migrate sanity trigger ${trigger.trigger}: ${e.message}`
            );
          }
        }
      }
    ...
    if (tables.some((t) => t.name === "email_verifications")) {
      ...
      const verifications = sqlite
        .prepare("SELECT * FROM email_verifications")
        .all() as any[];
          ...
          await prisma.character.create({
            data: {
              ...
              updatedAt: char.updated_at
                ? new Date(char.updated_at)
                : new Date(),
              instantiatedFrom: char.instantiated_from,
              inheritsKnowledge: char.inherits_knowledge
                ? JSON.parse(char.inherits_knowledge)
                : null,
    ...
    if (tables.some((t) => t.name === "npc_relationships")) {
      ...
      const relationships = sqlite
        .prepare("SELECT * FROM npc_relationships")
        .all() as any[];
    ...
    if (tables.some((t) => t.name === "module_backgrounds")) {
      ...
      const modules = sqlite
        .prepare("SELECT * FROM module_backgrounds")
        .all() as any[];
        ...
        try {
          await prisma.moduleBackground.create({
            data: {
              ...
              initialScenarioNpcs: mod.initial_scenario_npcs
                ? JSON.parse(mod.initial_scenario_npcs)
                : null,
              introduction: mod.introduction,
              tags: mod.tags ? JSON.parse(mod.tags) : null,
              emailId: mod.email_id,
              macroSceneStructure: mod.macro_scene_structure
                ? JSON.parse(mod.macro_scene_structure)
                : null,
              truthTimeline: mod.truth_timeline
                ? JSON.parse(mod.truth_timeline)
                : null,
              knowledgeMatrix: mod.knowledge_matrix
                ? JSON.parse(mod.knowledge_matrix)
                : null,
              redHerrings: mod.red_herrings
                ? JSON.parse(mod.red_herrings)
                : null,
              historicalMythos: mod.historical_mythos
                ? JSON.parse(mod.historical_mythos)
                : null,
              globalTrigger: mod.global_trigger
                ? JSON.parse(mod.global_trigger)
                : null,
              endStateDefinition: mod.end_state_definition
                ? JSON.parse(mod.end_state_definition)
                : null,
    ...
    if (tables.some((t) => t.name === "scenarios")) {
      ...
      const scenarios = sqlite
        .prepare("SELECT * FROM scenarios")
        .all() as any[];
      for (const scenario of scenarios) {
        const resolvedModuleId =
          (typeof scenario.module_id === "string" &&
          scenario.module_id.length > 0
            ? scenario.module_id
            : undefined) ||
        ...
        if (!resolvedModuleId) {
          warn(
            `  Failed to migrate scenario ${scenario.scenario_id}: missing module_id mapping`
          );
          continue;
        }
        try {
          await prisma.scenario.create({
            data: {
              ...
              connections: scenario.connections
                ? JSON.parse(scenario.connections)
                : null,
          ...
          if (e.code === "P2002") {
            ...
            warn(
              `  Failed to migrate scenario ${scenario.scenario_id}: ${e.message}`
            );
          }
        }
      }
    ...
    if (tables.some((t) => t.name === "scenario_snapshots")) {
      ...
      const snapshots = sqlite
        .prepare("SELECT * FROM scenario_snapshots")
        .all() as any[];
      for (const snapshot of snapshots) {
        const resolvedModuleId =
          (typeof snapshot.module_id === "string" &&
          snapshot.module_id.length > 0
            ? snapshot.module_id
            : undefined) ||
        ...
        if (!resolvedModuleId) {
          warn(
            `  Failed to migrate scenario snapshot ${snapshot.snapshot_id}: missing module_id mapping`
          );
          continue;
        }
        try {
          await prisma.scenarioSnapshot.create({
            data: {
              ...
              showMap:
                snapshot.show_map !== null ? Boolean(snapshot.show_map) : null,
    ...
    if (tables.some((t) => t.name === "scenario_characters")) {
      ...
      const chars = sqlite
        .prepare("SELECT * FROM scenario_characters")
        .all() as any[];
      for (const char of chars) {
        ...
        if (!resolvedModuleId) {
          warn(
            `  Failed to migrate scenario character ${char.id}: missing module_id mapping`
          );
          continue;
        }
        try {
    ...
    if (tables.some((t) => t.name === "scenario_clues")) {
      ...
      const clues = sqlite
        .prepare("SELECT * FROM scenario_clues")
        .all() as any[];
      for (const clue of clues) {
        const resolvedModuleId =
        ...
        if (!resolvedModuleId) {
          warn(
            `  Failed to migrate scenario clue ${clue.clue_id}: missing module_id mapping`
          );
          continue;
        }
        try {
    ...
    if (tables.some((t) => t.name === "scenario_conditions")) {
      ...
      const conditions = sqlite
        .prepare("SELECT * FROM scenario_conditions")
        .all() as any[];
      for (const condition of conditions) {
        const resolvedModuleId =
          (typeof condition.module_id === "string" &&
          condition.module_id.length > 0
            ? condition.module_id
            : undefined) ||
        ...
        if (!resolvedModuleId) {
          warn(
            `  Failed to migrate scenario condition ${condition.condition_id}: missing module_id mapping`
          );
          continue;
        }
        try {
      ...
      for (const session of sessions) {
        ...
        try {
          ...
          if (e.code === "P2002") {
            ...
            warn(
              `  Failed to migrate session ${session.session_id}: ${e.message}`
            );
          }
        }
      }
    ...
    if (tables.some((t) => t.name === "game_turns")) {
      ...
      const turns = sqlite
        .prepare("SELECT * FROM game_turns ORDER BY turn_number ASC")
        .all() as any[];
            ...
            try {
              await prisma.gameTurn.create({
                data: {
                  ...
                  actionAnalysis: turn.action_analysis
                    ? JSON.parse(turn.action_analysis)
                    : null,
                  actionResults: turn.action_results
                    ? JSON.parse(turn.action_results)
                    : null,
                  directorDecision: turn.director_decision
                    ? JSON.parse(turn.director_decision)
                    : null,
                  keeperNarrative: turn.keeper_narrative,
                  clueRevelations: turn.clue_revelations
                    ? JSON.parse(turn.clue_revelations)
                    : null,
    ...
    if (tables.some((t) => t.name === "game_checkpoints")) {
      ...
      const checkpoints = sqlite
        .prepare("SELECT * FROM game_checkpoints")
        .all() as any[];
      for (const cp of checkpoints) {
        try {
          ...
          if (e.code === "P2002") {
            ...
            warn(
              `  Failed to migrate checkpoint ${cp.checkpoint_id}: ${e.message}`
            );
          }
        }
      }
    ...
    if (tables.some((t) => t.name === "user_token_usage")) {
      ...
      const usage = sqlite
        .prepare("SELECT * FROM user_token_usage")
        .all() as any[];
    ...
    info("Next steps:");
    console.log(
      '  1. Verify data in PostgreSQL: psql -d coc_game_db -c "SELECT COUNT(*) FROM users;"'
    );
    console.log("  2. Test application functionality");
    console.log(
      "  3. Backup SQLite database: tar -czf sqlite-backup.tar.gz data/"
    );
