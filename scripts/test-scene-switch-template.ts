...
function injectActionLogs(
  npcs: DynamicNPCProfile[],
  initialScene: DynamicScenarioSnapshot
) {
  console.log(
    "\n💉 Injecting action logs into NPCs in scene:",
    initialScene.name
  );
  ...
  console.log(
    `   Found ${npcIdsInScene.length} NPCs in scene: ${npcIdsInScene.join(", ")}`
  );
  ...
  const sampleActions = [
    {
      ...
      summary:
        "Checked the emergency lighting system in the corridor. Found that backup batteries are depleting faster than expected.",
    },
    {
      time: "Day 3, 01:55",
      location: initialScene.location,
      summary:
        "Heard strange sounds from the lower decks. Metallic scraping and what sounded like water dripping.",
    },
    {
      time: "Day 3, 02:00",
      location: initialScene.location,
      summary:
        "Noticed temperature drop in the area. Despite the tropical climate, breath is now visible.",
    },
  ];
  ...
  for (const npcId of npcIdsInScene) {
    ...
    if (npc) {
      console.log(
        `   ✓ Injecting ${sampleActions.length} actions into ${npc.name} (${npc.id})`
      );
...
function generateTemplateContext(
  ...
  console.log(
    "\n🔧 Generating template context for target scene:",
    targetScenarioName
  );
  ...
  const targetScenario = allSnapshots.find(
    (s) => s.name === targetScenarioName
  );
  if (!targetScenario) {
  ...
  const templateContext = {
    ...
    previousGlobalTriggerJson: state.globalTrigger
      ...
      : null,
    endStateJson: state.endState
      ? JSON.stringify(state.endState, null, 2)
      : "null",
  };
...
async function main() {
  ...
  // Load module data
  const { macroScene, truthTimeline, knowledgeMatrix, redHerrings, scenarios } =
    ...
    actionLog: [
      {
        ...
        summary:
          "Woke up to complete darkness in cabin. Power completely out, emergency lights flickering.",
      },
      {
        time: "Day 3, 01:45",
        location: "Grand Atrium",
        summary:
          "Made way to Grand Atrium. Found hundreds of passengers in panic.",
      },
    ],
  };
  ...
  for (const item of parsed.updatedSnapshots) {
    const safeName = (item.scenarioId || "unknown").replace(
      /[^a-zA-Z0-9_-]/g,
      "_"
    );
    ...
    console.log(
      `   • ${fileName} (${item.isTargetScene ? "target" : "background"})`
    );
  }
  ...
  console.log(
    `  • updatedSnapshots: ${output.updatedSnapshots.length} items → each in scripts/test/*.json`
  );
  if (output.globalTrigger) {
    console.log(`  • globalTrigger: present`);
  }
  console.log(
    `  • _metadata: currentScene=${output._metadata.currentScene}, targetScene=${output._metadata.targetScene}`
  );
  console.log(`  • Full result: ${OUTPUT_FILE}`);

  console.log("\n📊 NPCs with injected action logs:");
  for (const npc of state.npcCharacters.filter(
    (n) => n.actionLog && n.actionLog.length > 0
  )) {
    console.log(
      `  • ${npc.name} (${npc.id}): ${(npc.actionLog ?? []).length} actions`
    );
  }
}
