...
├── calls: tickProcessor.executeSingleTick(mode: "simulation")
└── emits: SimulationEvent via EventEmitter → WebSocket

tickProcessor.executeSingleTick (refactored)
├── mode: "player_turn" | "simulation"
...
1. Call `tickProcessor.executeSingleTick(mode: "simulation")`
2. Collect emitted events from tick result
3. Persist events to `simulation_events` table
4. Emit events via EventEmitter → WebSocket
...
6. If game time crosses midnight → call `NPCPlanningAgent.onNewDay()` (summarize memories, generate new daily schedules)
...
Add a `mode` parameter to `executeSingleTick()`.
...
In simulation mode, NPCs discover clues when their actions succeed at relevant locations. Same `discoverClues()` logic but:
- Acting NPC is the discoverer (not a player character)
- Discovered clues are stored per-NPC in the NpcMemory framework (type: `clue`)
- Clue difficulty gating based on NPC action success level (same thresholds)
