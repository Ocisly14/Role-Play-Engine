...
describe("weatherFeature", () => {
  ...
  describe("HP drain", () => {
    it("should not drain HP when intensity < 3", () => {
      runTicks(dgsm, runtime, 1); // init
      dgsm._addNpc("npc_guard", "town_square", 10);

      const ws = dgsm.getFeatureSceneState("weather", "town") as any;
      ws.weatherType = "extreme_cold";
      ws.intensity = 2; // below threshold
      ws.ticksInState = 0;
      dgsm.setFeatureSceneState("weather", "town", ws);

      runTicks(dgsm, runtime, 6);

      expect(dgsm.getState().npcStats["npc_guard"].hp).toBe(10);
    });

    it("should drain NPC HP when extreme cold intensity >= 3 and CON check fails", () => {
      runTicks(dgsm, runtime, 1); // init
      dgsm._addNpc("npc_guard", "town_square", 10);
      ...
      ws.weatherType = "extreme_cold";
      ws.intensity = 4;
      ws.ticksInState = 1; // transition fires at tick 5, not 6
      dgsm.setFeatureSceneState("weather", "town", ws);

      const mockRandom = vi.spyOn(Math, "random");
      // Tick 5: transition check fires (ticksInState reaches 6)
      // town: sampleTransition(extreme_cold) — cold row cumsum: [0.10,0.10,0.15,0.20,0.40,0.40,1.00], 0.90 → cold stays
      mockRandom.mockReturnValueOnce(0.90);
      // evolveIntensity: 0.30 → no change
      mockRandom.mockReturnValueOnce(0.30);
      // forest: sampleTransition(clear) — 0.30 → clear stays
      mockRandom.mockReturnValueOnce(0.30);

      // Tick 6: no transition check (ticksInState = 1), exposure reaches 6
      // processExposureDrain CON check: failChance = 0.3 + (4-3)*0.15 = 0.45
      // 0.10 < 0.45 → fail → -1 HP
      mockRandom.mockReturnValueOnce(0.10);

      runTicks(dgsm, runtime, 6);

      expect(dgsm.getState().npcStats["npc_guard"].hp).toBe(9);

      mockRandom.mockRestore();
    });

    it("should drain player HP when in affected outdoor scene", () => {
      runTicks(dgsm, runtime, 1); // init
      dgsm._setCurrentScene("town_square");
      ...
      const mockRandom = vi.spyOn(Math, "random");
      // Tick 5: transition check
      // town: sampleTransition(extreme_heat) — heat row cumsum: [0.25,0.25,0.30,0.33,0.33,1.00,1.00], 0.90 → heat stays
      mockRandom.mockReturnValueOnce(0.90);
      // evolveIntensity: 0.30 → no change
      mockRandom.mockReturnValueOnce(0.30);
      // forest: sampleTransition(clear) — 0.30 → clear stays
      ...
      // Tick 6: no transition, exposure reaches 6
      // failChance = 0.3 + (5-3)*0.15 = 0.60, 0.10 < 0.60 → fail → -1 HP
      mockRandom.mockReturnValueOnce(0.10);
    ...
    it("should clean up exposure when character leaves region", () => {
      runTicks(dgsm, runtime, 1); // init
      dgsm._addNpc("npc_guard", "town_square", 10);
      ...
      // Run 3 ticks to accumulate exposure
      runTicks(dgsm, runtime, 3);

      let wsUpdated = dgsm.getFeatureSceneState("weather", "town") as any;
      expect(wsUpdated.exposureTicks["npc_guard"]).toBe(3);

      // Move NPC out of region
      dgsm.getState().npcLocations["npc_guard"] = "forest_path";

      runTicks(dgsm, runtime, 1);

      wsUpdated = dgsm.getFeatureSceneState("weather", "town") as any;
      expect(wsUpdated.exposureTicks["npc_guard"]).toBeUndefined();
    });
  });
