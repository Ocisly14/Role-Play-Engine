...
- CRITICAL: actionLog \`location\` MUST exactly match one of the scenario \`name\` values from the Scenarios input (or the player scene \`name\` for SuddenActionLogs). Do NOT use sub-locations, room names, or paraphrased names. The system uses exact string matching to track NPC positions.
  ...
  "SuddenActionLogs": [
    {
      ...
      "actionLog": [
        {
          "time": "Day X, HH:MM",
          "location": "exact player scene name from input",
...
- characters must be generated by checking the suddenActionLogs and baseline: only include an NPC whose LATEST actionLog entry has a location matching this scene's name. NPCs who passed through but whose latest action is at a different scene must NOT be included.
- CRITICAL: All \`location\` fields (in characters, reactionNpcActionLogUpdates) must exactly match a scenario \`name\`. The system uses exact string matching.
