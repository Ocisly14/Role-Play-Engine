...
- CRITICAL: actionLog \`location\` MUST exactly match one of the scenario \`name\` values from the Scenarios input. Do NOT use sub-locations, room names, or paraphrased names. The system uses exact string matching to track NPC positions.
      ...
      "npcActionLogUpdates": [
        {
          ...
          "actionLog": [
            {
              "location": "exact scenario name from input",
...
- targetSnapshot must include: description, gameTime, characters, items, conditions, keeperNotes, showMap (if needed), and optional connections update.
- description must be a concise atmospheric narrative of at most 2 sentences, covering key sensory details and what changed in the time window.
- characters must be generated by checking the actionTimeline: only include an NPC whose LATEST actionLog entry (within the full timeline, not just this window) has a location matching this target scene's name. NPCs who passed through but whose latest action is at a different scene must NOT be included.
- CRITICAL: All \`location\` fields in snapshot characters must exactly match the scenario \`name\`. The system uses exact string matching.
- each character item in snapshot.characters must contain:
  ...
  - location
  - notes
- items must be full Item objects (id, name, description, type, category, reveals, discoveryMethod, damaged, damageDetails when applicable). Evidence items use \`category: "evidence"\`.
- **Item ID & State Stability (CRITICAL)**:
  - Every item carried over from baseline MUST keep its original \`id\` unchanged. Do NOT generate new IDs for existing items.
  - Do NOT change the \`damaged\` fields of baseline items. These are managed by game mechanics, not by snapshot generation.
  - NPCs may move an item to a different scene — keep the same \`id\`.
  - You may add genuinely new items (with new IDs) if NPC actions created new evidence. Do not fabricate items without timeline justification.
  - If a baseline item no longer exists in this scene (NPC moved/destroyed it), omit it from this snapshot's items array — it should appear in the destination scene's snapshot instead, with the same \`id\`.
...
- items and conditions must be inferred from BOTH NPC timeline + player action log in this window.
...
- items guidance:
  ...
  - Keep item objects structurally valid (id, name, description, type, category, reveals, discoveryMethod, damaged, damageDetails when applicable). Evidence items use \`category: "evidence"\`.
  - **Item ID & State Stability (CRITICAL)**: Every baseline item MUST keep its original \`id\`. Do NOT generate new IDs for existing items. Do NOT change \`damaged\` fields — these are managed by game mechanics. NPCs may move an item to another scene (keep same \`id\`). If an item was removed from this scene by NPC action, omit it — it should appear in the destination scene with the same \`id\`.
  ...
  "updatedSimplifiedSnapshots": [
    {
      ...
      "snapshot": {
        ...
        "name": "scene name",
        "location": "scene location",
        "description": "concise 1-2 sentence updated description",
        "gameTime": "Day X, HH:MM",
        "items": [],
        "conditions": []
      },
