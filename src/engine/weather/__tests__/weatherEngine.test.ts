...
const { judgeWeather, renderWeatherRequest } = await import(
  "../weatherEngine.js"
);
...
describe("judgeWeather", () => {
  ...
  it("sends the errors back once and takes the corrected judgement", async () => {
    generateToolCalls
      .mockResolvedValueOnce(
        submission({
          blocks: [{ connectionId: "connection.ridge.pass", reason: "x" }],
          conditions: [],
        })
      )
      .mockResolvedValueOnce(
        submission({ blocks: [], conditions: [] }, "call_2")
      );
    const result = await judgeWeather(request);
    expect(result).toEqual({
      ok: true,
    ...
    const second = generateToolCalls.mock.calls[1][0]
  ...
  it("gives up after one repair", async () => {
    const bad = submission({
      blocks: [{ connectionId: "nope", reason: "x" }],
      conditions: [],
    });
