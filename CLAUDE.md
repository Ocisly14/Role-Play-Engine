...
**World Action Engine** (`engine/resolution/worldActionEngine.ts`) — a fixed pipeline of six ordered phases per triggered tick, over the full world context: `endings → starts → characterChanges → itemChanges → sceneChanges → occurrences`. Each phase is a separate request carrying only its own English rule modules and exactly one strict submission tool (`submit_endings`, `submit_starts`, `submit_character_changes`, `submit_item_changes`, `submit_scene_changes`, `submit_occurrences` — `worldResolutionStageSchemas.ts`); the endings phase alone also offers `damageRoll`. A phase's array is validated the moment it lands (`worldResolutionStageValidator.ts`) before it becomes read-only input to every later phase's prompt. A rejection is answered through the same tool, up to `MAX_PHASE_ATTEMPTS` (3) attempts, and never revises a phase already accepted; there is no patch tool. Two phases are corrected by difference: in `starts` (rows keyed by actionId) and `occurrences` (rows keyed by cited actions + speech flag) code keeps every row that passed on its own, shows the model what it kept and what is still owed, and merges the answer before validating the whole array again (`MERGE_PHASES`, `retainedRows`, `mergeRows`). The other four phases resubmit their complete array. Measured reason: asked for the complete array, the model shrank it — six start entries to two, four occurrence rows to two with placeholder content — and deleted the action a field was missing from instead of adding the field. There are **no action types and no per-action prompts** — one rule document (`engine/rules/world-action-resolution.md`) governs everything, with `engine/rules/session-protocol.md` (rendered per phase by `worldResolutionStagePrompts.ts`) as the phase contract.

Once all six phases are accepted, the assembled draft passes through the same global gate exactly once — `validateRawResolution` then `finalizeResolution` — before anything reaches the Applier; **no phase mutates `DynamicGameState` on its own**. A global-validation failure rewinds to the earliest phase that owns the fault (each error kind maps to one phase), discards that phase and every phase downstream of it, and reruns forward with the global errors folded into the rewound phase's prompt; at most one rewind (`MAX_GLOBAL_REWINDS` = 1) is allowed per tick, and a draft still globally invalid after it applies nothing. The whole resolution shares one hard ceiling of `MAX_PROVIDER_CALLS` (12) provider calls — every phase attempt, the strict-schema fallback retry, and the rewind's reruns all draw from the same budget; exhausting it rejects the tick.

The session's economics are still part of its design — a phase's own array is narrow, but the stable world context is currently resent to every phase — hence exactly **one** code tool anywhere in the pipeline, `damageRoll` (`engine/tools/diceTools.ts`, registered flat by name in `registerDefaults.ts`), because a roll must never be the model's. Pathfinding, movement cost and inventory lookups were deliberately removed: the request already carries the graph, the places and the items, so the model reads rather than queries. Six small strict schemas replace the old two-tool split (`submit_actions` was `strict: true`; `submit_effects` was not, because its three operation unions carried 19 `anyOf` branches the grammar compiler refused) — each phase schema is small enough to stay strict on its own. The one narrow exception: a deterministic provider-side grammar/schema compilation rejection (never a transport error, a rate limit, malformed model output, or a validation failure) triggers exactly one warned retry of that phase with an otherwise-identical non-strict copy of its tool, and the downgrade is remembered for the rest of the process by `(provider, model, schema fingerprint)` so it isn't re-paid every tick. `scripts/probe-strict-schema.ts` still re-measures the strict ceilings against the live API for the cost of a few rejected requests; don't change a phase schema without running it.
