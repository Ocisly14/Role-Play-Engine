...
- **控制大厅**(内门之后,不再是 1985):环形操作台,这个年代不该存在的玻璃平板上
  滚动着天气、季节、时刻 —— 灰港的雾在这里是一个旋钮。(工程同构:天气子系统与 `gameClock`。)
- **档案室**:全体镇民的档案 —— 照片、性格参数、"出厂背景记忆"的打印稿;
  Earl 的档案上盖着"多轮保留"的章;1889 年海难作为"背景事件模板"的原始设计稿也在这里。
  一个角色读到自己的档案,就是真相揭露的定义性瞬间。(工程同构:角色档案与 `context` 记忆。)
- **维护间**:一排挂着的黑西装,尺寸各异,下面摆着同款黑皮鞋。没有别的。
  (维护进程的"物料"。)
...
主街按南北拆为两段(移动成本挂在边上,单场景内部没有距离,拆段才能承载"纵贯 30 分钟"):

| 大场景 | 说明 |
|---|---|
| 主街北段 | 户外;临街:蓝鸟餐馆、杂货店兼邮局、警长办公室;引出旧海岸公路(旅馆/车行道)、Holt 家、林道口 |
| 主街南段 | 户外;临街:街机厅、电视维修铺、诊所;引出后街(Reyes 家)、下坡道(码头) |
...
**镇区连接边(步行分钟)**:主街北段—主街南段 15′;临街店铺—所在街段 各 2′;
主街南段—Reyes 家 8′(后街);主街南段—码头/海滩 15′(下坡道);码头—Earl 小屋 5′;
码头/海滩—灯塔崖 30′(崖间小径,唯一通路);主街北段—Holt 家 20′;
主街北段—汽车旅馆 25′(旧海岸公路);汽车旅馆—自动大门 车行道(货车 30′,步行约 4.5 小时);
自动大门—雷德利站装卸区 货车 5′(步行 15′);主街北段—林道口 10′。
...
- [ ] **罐头厂废墟**要不要作为码头/海滩场景内的地标(不做独立大场景)。利:孩子团天然的探险点;弊:分流动线,削弱围栏磁力
...
- [ ] **真相空间封闭**:站内基地三场景(控制大厅/档案室/维护间)存在于模组数据但开局不可达、内容不入任何上下文 —— 需要的加载/准入机制确认
