...
- [x] **车辆速度档**:载具已实现为**可移动的感知边界** —— 对外是所在位置的 item 形存在,对内是驾驶室场景(乘客免费随行:车动人不动);司机在舱内即可驾驶(无状态绑定,"有没有资格开"由 Engine 全上下文裁),路线仍由角色说出。可行驶性与速度编码为每条路的可选 `driveTimeMinutes`(车行道 30′/站前引道 5′/旧海岸公路 3′/主街 2′/霍尔特巷 2′;林径、崖径、后巷不可行驶)。Frank 的货车(`VEH_frank_truck`)停在 Holt 家院门,驾驶室三件物齐备
