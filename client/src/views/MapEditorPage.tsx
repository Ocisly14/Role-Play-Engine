...
export default function MapEditorPage() {
    ...
    if (
      ...
      setRects((prev) => [
        ...prev,
        {
          id,
          name: id,
          x: Math.round(drawPreview.x),
          y: Math.round(drawPreview.y),
          width: Math.round(drawPreview.w),
          height: Math.round(drawPreview.h),
        },
      ]);
      setSelectedId(id);
    }
  ...
  function updateSelected(u: Partial<Rect>) {
    setRects((prev) =>
      prev.map((r) => (r.id === selectedId ? { ...r, ...u } : r))
    );
  }
      ...
      <div className="w-56 bg-gray-900 border-r border-gray-700 flex flex-col p-3 gap-3 overflow-y-auto">
        ...
        <div className="space-y-1">
          ...
          <div className="flex gap-1">
            <input
              className="flex-1 bg-gray-800 text-xs px-2 py-1 rounded border border-gray-700"
            ...
            <button
              onClick={() =>
                bgInput &&
                loadBackground(
                  bgInput.startsWith("http")
                    ? bgInput
                    : `/api/maps/scene/${bgInput}`
                )
              }
        ...
        <div className="space-y-1">
          ...
          {sceneIds.map((sid) => (
            <button
              ...
              className={`w-full text-left text-xs px-2 py-1.5 rounded ${currentSceneId === sid ? "bg-green-600" : "bg-gray-800 hover:bg-gray-700"}`}
            >
              {sid}
            </button>
          ))}
          <div className="flex gap-1">
            <input
              className="flex-1 bg-gray-800 text-xs px-2 py-1 rounded border border-gray-700"
              placeholder="SCN_X_SUB_Y"
              value={newSceneId}
              onChange={(e) => setNewSceneId(e.target.value)}
            />
            <button
              onClick={() => {
                if (newSceneId && !sceneIds.includes(newSceneId)) {
                  setSceneIds((p) => [...p, newSceneId]);
                  setNewSceneId("");
                }
              }}
        ...
        <div className="space-y-1">
          ...
          <button
            ...
            className={`w-full text-left text-xs px-2 py-1.5 rounded ${tool === "select" ? "bg-purple-600" : "bg-gray-800 hover:bg-gray-700"}`}
          >
            Select / Pan
          </button>
          <button
            onClick={() => setTool("npc_area")}
            className={`w-full text-left text-xs px-2 py-1.5 rounded ${tool === "npc_area" ? "bg-purple-600" : "bg-gray-800 hover:bg-gray-700"}`}
          >
            Draw NPC Area
          </button>
        </div>
        ...
        <div className="space-y-1 flex-1 overflow-y-auto">
          <label className="text-xs text-gray-500">
            NPC Areas ({rects.length})
          </label>
          {rects.map((r) => (
            <div
              key={r.id}
              onClick={() => setSelectedId(r.id)}
              className={`text-xs px-2 py-1 rounded cursor-pointer ${r.id === selectedId ? "bg-gray-700" : "bg-gray-800 hover:bg-gray-750"}`}
            >
              <span
                className="w-2 h-2 rounded-full inline-block mr-1"
                style={{ backgroundColor: AREA_BORDER }}
              />
              {r.name}
            </div>
          ))}
        ...
        <div className="space-y-1 pt-2 border-t border-gray-700">
          <button
            ...
            className="w-full bg-green-600 text-xs px-2 py-2 rounded hover:bg-green-500 font-medium"
          >
            Export JSON
          </button>
          <button
            onClick={copyConfig}
            className="w-full bg-gray-700 text-xs px-2 py-2 rounded hover:bg-gray-600"
          >
            Copy to Clipboard
          </button>
        </div>
      ...
      <div className="w-52 bg-gray-900 border-l border-gray-700 p-3 space-y-3 overflow-y-auto">
        ...
        {selectedRect ? (
          <div className="space-y-2">
            ...
            <Field
              label="X"
              value={String(Math.round(selectedRect.x))}
              onChange={(v) => updateSelected({ x: Number(v) })}
            />
            <Field
              label="Y"
              value={String(Math.round(selectedRect.y))}
              onChange={(v) => updateSelected({ y: Number(v) })}
            />
            <Field
              label="Width"
              value={String(Math.round(selectedRect.width))}
              onChange={(v) => updateSelected({ width: Number(v) })}
            />
            <Field
              label="Height"
              value={String(Math.round(selectedRect.height))}
              onChange={(v) => updateSelected({ height: Number(v) })}
            />
            <button
              onClick={deleteSelected}
              className="w-full bg-red-700 text-xs px-2 py-1.5 rounded hover:bg-red-600 mt-2"
            >
              Delete
            </button>
          </div>
...
function Field({
  ...
  return (
    <div>
      ...
      <input
        className="w-full bg-gray-800 text-xs px-2 py-1 rounded border border-gray-700 disabled:opacity-50"
        value={value}
