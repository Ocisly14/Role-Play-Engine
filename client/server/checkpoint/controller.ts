...
export async function saveCheckpoint(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!dynamicGameState) {
      ...
      res.status(400).json({ error: "DynamicWorld game not started. Please start the game first." });
      return;
    }
    ...
    console.log(`[${new Date().toISOString()}] [Checkpoint Save] Game state found - Type: DynamicWorld, Character: ${characterName} (ID: ${characterId}), Session: ${sessionId}`);

    if (!characterId) {
      console.log(`[${new Date().toISOString()}] [Checkpoint Save] ERROR: No character ID in game state. playerCharacter: ${JSON.stringify(dynamicGameState.playerCharacter)}`);
...
export async function loadCheckpointData(req: Request, res: Response): Promise<void> {
  try {
    ...
    if (!gameStateAny.moduleName || gameStateAny.updatedDynamicScenarioSnapshots === undefined) {
      res.status(400).json({ error: "Only DynamicWorld checkpoints are supported" });
      return;
    }
